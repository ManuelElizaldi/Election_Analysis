...
file_to_load = os.path.join("Week3", "election_results.csv")
# Add a variable to save the file to a path.
file_to_save = os.path.join("Week3", "election_analysis.txt")
...
with open(file_to_save, "w") as txt_file:
    ...
    for county, votes in county_list.items():
        print(county, votes)
        # 6b: Retrieve the county vote count.
        county_vote_count = county_votes[county_name]
        # 6c: Calculate the percentage of votes for the county.
        county_vote_percentage = float(county_vote_percentage) / float(total_votes) * 100
            county_results =(
            f"{county_name}: {vote_percentage:.1f}% ({county_vote_percentage:,})\n")
